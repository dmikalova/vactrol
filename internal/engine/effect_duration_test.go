...
func TestForDurationValidate(t *testing.T) {
	...
	if err := (ForDuration{Duration: NextTurn, Effects: []Effect{house, cannotDamage}}).validate(); err == nil {
	...
	badChild := CannotBeDealtDamage{Duration: EndOfTurn} // no target
	...
	if err := (ForDuration{Duration: EndOfTurn, Effects: []Effect{house, cannotDamage}}).validate(); err != nil {
...
func TestForDurationTextSharedSubject(t *testing.T) {
	e := ForDuration{Duration: EndOfTurn, Effects: []Effect{
		...
		CannotBeDealtDamage{Target: Target{Kind: TargetTriggeringCreature}, Duration: EndOfTurn},
	}}
...
func TestForDurationTextDistinctSubjects(t *testing.T) {
	e := ForDuration{Duration: EndOfTurn, Effects: []Effect{
		...
		CannotBeDealtDamage{Target: Target{Kind: TargetEachFriendlyCreature}, Duration: EndOfTurn},
	}}
	got := e.Text()
	if !strings.Contains(
		got,
...
func TestForDurationResolvesChildren(t *testing.T) {
	...
	e := ForDuration{Duration: EndOfTurn, Effects: []Effect{
		...
		CannotBeDealtDamage{Target: Target{Kind: TargetThisCreature}, Duration: EndOfTurn},
	}}
