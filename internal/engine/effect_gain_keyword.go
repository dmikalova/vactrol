...
// grant is held on each creature and lifts at that player's ready phase, so it
// survives the opponent's turn — which is what a defensive keyword like elusive
// needs. A keyword that only matters on your own turn (Scout's Skirmish) uses the
