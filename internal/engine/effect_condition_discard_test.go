...
func TestCardsInDiscardAtLeast(t *testing.T) {
	if got := (CardsInDiscardAtLeast{House: Untamed, Type: Creature, Amount: 3}).
		CondText(); got != "if there are 3 or more Untamed creatures in your discard pile" {
		t.Errorf("text = %q", got)
	}
	if (CardsInDiscardAtLeast{House: Untamed, Type: Creature}).validate() == nil {
	...
	e := CardsInDiscardAtLeast{House: Untamed, Type: Creature, Amount: 3}
	...
	if (CardsInDiscardAtLeast{House: Untamed, Type: Creature, Amount: 3}).
