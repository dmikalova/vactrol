...
func TestPhoenixHeart(t *testing.T) {
	t.Run("returns its host to hand and deals 3 to each creature when the host is destroyed", func(t *testing.T) {
		...
		h := ct.Play(t, ct.Setup{
			P1: ct.Side{
				...
				InPlay: ct.Cards(
					...
					ct.Bind(&bystander, ct.Creature(ct.OfHouse(card.House.Brobnar), ct.Power(5))),
				),
				Hand: ct.Cards(PhoenixHeart),
			},
			P2: ct.Side{
				InPlay: ct.Cards(ct.Bind(&enemy, ct.Creature(ct.OfHouse(card.House.Brobnar), ct.Power(5)))),
			},
		})
