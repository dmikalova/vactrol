...
func TestSneklifter(t *testing.T) {
	t.Run("takes control of an enemy artifact and reassigns an off-identity one to Shadows", func(t *testing.T) {
		var relic ct.Card
		h := ct.Play(t, ct.Setup{
			P1: ct.Side{House: card.House.Shadows, Hand: ct.Cards(Sneklifter)},
			P2: ct.Side{InPlay: ct.Cards(ct.Bind(&relic, ct.Artifact(ct.OfHouse(card.House.Untamed))))},
		})
		h.Game().SetPlayerHouses(0, []engine.House{card.House.Shadows, card.House.Logos, card.House.Sanctum})
	...
	t.Run("keeps an on-identity artifact's house unchanged", func(t *testing.T) {
		...
		h := ct.Play(t, ct.Setup{
			P1: ct.Side{House: card.House.Shadows, Hand: ct.Cards(Sneklifter)},
			P2: ct.Side{InPlay: ct.Cards(ct.Bind(&relic, ct.Artifact(ct.OfHouse(card.House.Mars))))},
		})
		h.Game().SetPlayerHouses(0, []engine.House{card.House.Shadows, card.House.Mars, card.House.Sanctum})
