	...
	t.Run("archives each friendly Knight creature in play", func(t *testing.T) {
		...
		h := ct.Play(t, ct.Setup{
			P1: ct.Side{
				...
				InPlay: ct.Cards(
					ct.Bind(&knight, ct.Creature(ct.OfHouse(card.House.Sanctum), ct.Traits("Knight"))),
					ct.Bind(&cleric, ct.Creature(ct.OfHouse(card.House.Sanctum), ct.Traits("Cleric"))),
				),
			},
			P2: ct.Side{InPlay: ct.Cards(
				ct.Bind(&enemyKnight, ct.Creature(ct.OfHouse(card.House.Sanctum), ct.Traits("Knight"))),
			)},
		})
