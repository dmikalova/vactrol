	...
	t.Run("grants the host +2 hazardous", func(t *testing.T) {
		...
		h := ct.Play(t, ct.Setup{
			P1: ct.Side{
				...
				InPlay: ct.Cards(ct.Bind(&attacker, ct.Creature(ct.OfHouse(card.House.Brobnar), ct.Power(2)))),
			},
			P2: ct.Side{
				InPlay: ct.Cards(
					ct.Upgraded(ct.Bind(&host, ct.Creature(ct.OfHouse(card.House.Sanctum), ct.Power(4))), ArmageddonCloak),
				),
			},
		})
