...
func TestStrangeGizmo(t *testing.T) {
	t.Run("destroys each creature and artifact after its controller forges a key", func(t *testing.T) {
		var ally, foe ct.Card
		h := ct.Play(t, ct.Setup{
			P1: ct.Side{
				House:  card.House.Logos,
				Amber:  6,
				InPlay: ct.Cards(StrangeGizmo, ct.Bind(&ally, ct.Creature(ct.OfHouse(card.House.Logos), ct.Power(3)))),
			},
