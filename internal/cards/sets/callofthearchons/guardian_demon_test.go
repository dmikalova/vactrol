...
func TestGuardianDemon(t *testing.T) {
	t.Run("heals a creature and deals the healed amount to another", func(t *testing.T) {
		...
		h.P1.ClickCard(wounded) // heal the wounded creature
	...
	t.Run("still prompts to heal even when nothing is damaged", func(t *testing.T) {
		var foe ct.Card
		h := ct.Play(t, ct.Setup{
			P1: ct.Side{
				House: card.House.Dis,
		...
		// Heal offers every creature, so the reap still asks which to heal even
		// with nothing damaged. Healing an undamaged creature heals nothing, so
		// the follow-up damage amount is zero and no damage target is asked for.
		h.P1.Reap(GuardianDemon)
		h.P1.ClickCard(GuardianDemon)
