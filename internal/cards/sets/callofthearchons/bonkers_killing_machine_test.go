...
func TestBonkersKillingMachine(t *testing.T) {
	t.Run(
		...
		func(t *testing.T) {
			...
			h := ct.Play(t, ct.Setup{
				P1: ct.Side{
					...
					InPlay: ct.Cards(
						...
						ct.Bind(
							&shadowsCreature,
							ct.Creature(ct.OfHouse(card.House.Shadows), ct.Power(4)),
						),
					),
					Deck: ct.Cards(
			...
			h.P1.ExpectPrompt("Choose a creature or artifact of that card's house").
				Source("Bonkers Killing Machine")
	...
	t.Run("destroys itself when fewer than two cards are destroyed", func(t *testing.T) {
		...
		h := ct.Play(t, ct.Setup{
			...
			P2: ct.Side{
				InPlay: ct.Cards(
					ct.Bind(&bystander, ct.Creature(ct.OfHouse(card.House.Brobnar), ct.Power(4))),
				),
				Deck: ct.Cards(
