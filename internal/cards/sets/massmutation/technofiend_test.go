...
func TestTechnoFiend(t *testing.T) {
	...
	t.Run("steals 1 Æmber when destroyed", func(t *testing.T) {
		var fiend, foe ct.Card
		h := ct.Play(t, ct.Setup{
			P1: ct.Side{
				House:  card.House.Dis,
				InPlay: ct.Cards(ct.Bind(&fiend, TechnoFiend)),
			},
			P2: ct.Side{
				InPlay: ct.Cards(
