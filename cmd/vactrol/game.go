...
func (c *teaChooser) ChooseCreature(prompt string, candidates []game.LocalID) (game.LocalID, bool) {
	if len(candidates) == 0 {
		return 0, false
	}
...
type gameModel struct {
	...
	player    int
	phase     gamePhase
	allHouses []game.House // the deck's houses, sorted (house choices)
	...
	choosePrompt string
	chooseCands  []game.LocalID
	chooseCur    int

	fightAttacker game.LocalID
	fightCands    []game.LocalID
	fightCur      int

	actionCreature game.LocalID
...
func newGameModel(snd *sender, w, h int) gameModel {
	...
	g := game.NewGame("Player 1", "Player 2", seed)
...
func setupDecks(g *game.Game, seed int64) {
	pool := cards.All()
	for player := 0; player < 2; player++ {
		defs := make([]game.CardDefinition, 0, deckSize)
...
func deckHouses() []game.House {
	seen := map[game.House]bool{}
	var out []game.House
...
func (m gameModel) appendUpgrades(s []slot, host game.LocalID) []slot {
...
func (m gameModel) playSlot(sl slot) (gameModel, tea.Cmd) {
	def := m.g.Def(sl.id)
	if h := m.g.State.ActiveHouse; h != game.HouseNone && def.House != h {
...
func (m gameModel) startFight(att game.LocalID) (gameModel, tea.Cmd) {
...
func (m gameModel) openCreatureMenu(id game.LocalID) (gameModel, tea.Cmd) {
	m.actionCreature = id
...
func (m gameModel) highlight() (game.LocalID, bool, int) {
	switch m.phase {
	case phaseChoose:
...
func (m gameModel) selectedDef() *game.CardDefinition {
	switch m.phase {
	case phaseChoose:
		if len(m.chooseCands) > 0 {
...
func (m gameModel) pendingDef() *game.CardDefinition {
	hand := m.g.Hand(m.player)
...
func (m gameModel) discardView() string {
	...
	all := append(append([]game.LocalID{}, you...), opp...)
	...
	idx := 0
	writePile := func(label string, ids []game.LocalID) {
	...
	b.WriteString(indentText(game.RenderCardText(def)) + "\n")
...
func (m gameModel) scoreLine(player int, tag string) string {
	...
	line := fmt.Sprintf("%-20s keys %d/%d   Æmber %d",
		g.PlayerName(player)+" ("+tag+")", g.Keys(player), game.KeysToWin, g.Aember(player))
	if player == m.player {
		if h := g.State.ActiveHouse; h != game.HouseNone {
...
func (m gameModel) renderCardLine(def *game.CardDefinition, sel bool, line string) string {
	st := otherHouseStyle
	if h := m.g.State.ActiveHouse; h != game.HouseNone && def.House == h {
		st = activeHouseStyle
	}
...
func (m gameModel) renderCreatures(player int, selID game.LocalID, hasSel bool) string {
...
func (m gameModel) renderArtifacts(player int, selID game.LocalID, hasSel bool) string {
